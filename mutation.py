...
import simplification, miscellaneous, initialization

def insert_operator(expressions: Expression, tree: list, key: PRNGKey, readout: bool = False, unary_operators_prob: float = 0.0):
    ...
    operator_indices = jnp.ravel(jnp.argwhere(jnp.array([miscellaneous.is_operator(node, expressions) for node in nodes])))
    ...
    if operator_type>1-unary_operators_prob: #unary operator
        ...
        if readout:
            other_leaf = initialization.sample_readout_leaf(new_key3, expressions)
        else:
    ...
    return eqx.tree_at(lambda t: miscellaneous.key_loc(t, path), tree, new_tree)

def add_subtree(expressions: Expression, tree: list, key: PRNGKey, readout: bool = False):
    #Replace a leaf with a new subtree
    nodes = jax.tree_util.tree_leaves(tree)
    flat_tree_and_path = jax.tree_util.tree_leaves_with_path(tree)
    leaf_indices = jnp.ravel(jnp.argwhere(jnp.array([miscellaneous.is_leaf(node, expressions) for node in nodes])))

    key, new_key1, new_key2 = jrandom.split(key, 3)
    index = jrandom.choice(new_key1, leaf_indices)
    path = flat_tree_and_path[index][0][:-1]
    if readout:
        return eqx.tree_at(lambda t: miscellaneous.key_loc(t, path), tree, initialization.sample_readout_tree(new_key2, expressions, depth=3))
    else:
        return eqx.tree_at(lambda t: miscellaneous.key_loc(t, path), tree, initialization.grow_node(new_key2, expressions, depth=3))
...
def mutate_operator(expressions: Expression, tree: list, key: PRNGKey):
    ...
    nodes = jax.tree_util.tree_leaves(tree)
    operator_indicies = jnp.ravel(jnp.argwhere(jnp.array([miscellaneous.is_operator(node, expressions) for node in nodes])))
    flat_tree_and_path = jax.tree_util.tree_leaves_with_path(tree)
    ...
    if symbol in expressions.binary_operators:
        ...
        new_tree = eqx.tree_at(lambda t: miscellaneous.key_loc(t, path), tree, new_operator)
...
def prepend_operator(expressions: Expression, tree: list, key: PRNGKey, readout: bool = False, unary_operators_prob: float = 0.0):
...
def mutate_leaf(expressions: Expression, tree: list, key: PRNGKey, readout: bool = False):
    ...
    while new_leaf == [nodes[index]]:
        ...
        index = jrandom.choice(new_key1, leaf_indices)
        if readout:
            new_leaf = initialization.sample_readout_leaf(new_key2, expressions, sd=3)
        else:
            new_leaf = initialization.sample_leaf(new_key2, expressions, sd=3)

    path = flat_tree_and_path[index][0][:-1]
    return eqx.tree_at(lambda t: miscellaneous.key_loc(t, path), tree, new_leaf)

def mutate_constant(tree: list, key: PRNGKey):
    "Change the value of a constant leaf. The value is sampled close to the old value"
    nodes = jax.tree_util.tree_leaves(tree)
    constant_indicies = jnp.ravel(jnp.argwhere(jnp.array([miscellaneous.is_constant(node) for node in nodes])))
    ...
    key, new_key = jrandom.split(key)
    return eqx.tree_at(lambda t: miscellaneous.key_loc(t, path), tree, value+jrandom.normal(new_key))

def delete_operator(expressions: Expression, tree: list, key: PRNGKey, readout: bool = False):
    "Replace an operator with a new leaf"
    key, new_key = jrandom.split(key)
    if readout:
        new_leaf = initialization.sample_readout_leaf(new_key, expressions, sd=3)
    else:
...
def mutate_tree(mutation_probabilities: Array, expressions: Expression, tree: list, key: PRNGKey, max_init_depth: int, allow_simplification: bool = True, readout: bool = False):
    ...
    if sum([miscellaneous.is_constant(node) for node in jax.tree_util.tree_leaves(tree)]) == 0: #Tree does not contain constants, so exclude mutations that require constants
    ...
    if not allow_simplification: #Simplifying the tree is not possible, so exclude simplification
        _mutation_probabilities["simplify_tree"] = 0
    ...
    if mutation_type=="mutate_operator":
        ...
        new_tree = delete_operator(expressions, tree, new_key2, readout)
    elif mutation_type=="prepend_operator":
        new_tree = prepend_operator(expressions, tree, new_key2, readout)
    elif mutation_type=="insert_operator":
        new_tree = insert_operator(expressions, tree, new_key2, readout)
    elif mutation_type=="mutate_constant":
        new_tree = mutate_constant(tree, new_key2)
    elif mutation_type=="mutate_leaf":
        new_tree = mutate_leaf(expressions, tree, new_key2, readout)
    elif mutation_type=="sample_subtree":
        if readout:
            new_tree = initialization.sample_readout_tree(new_key2, expressions, depth=max_init_depth)
        else:
            new_tree = initialization.grow_node(new_key2, expressions, depth=max_init_depth)

    elif mutation_type=="add_subtree":
        new_tree = add_subtree(expressions, tree, new_key2, readout)
    elif mutation_type=="simplify_tree":
        simplified_tree = simplification.simplify_tree(tree)
        if not simplified_tree:
            #Simplification did not change the tree
            new_tree = mutate_tree(mutation_probabilities, expressions, tree, new_key2, max_init_depth, allow_simplification=False, readout=readout)
        else:
