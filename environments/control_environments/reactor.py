...
class StirredTankReactor(EnvironmentBase):
    def __init__(self, process_noise, obs_noise, n_obs = 3, n_targets = 1):
        ...
        self.init_lower_bounds = jnp.array([275, 350, 0.4])
        self.init_upper_bounds = jnp.array([300, 375, 0.6])
    ...
    def initialize_parameters(self, params, ts):
        ...
        self.W = self.obs_noise*jnp.eye(self.n_obs)*(jnp.array([10,10,0.05])[:self.n_obs])
    ...
    def sample_params(self, batch_size, mode, ts, key):
        if mode=="Constant":
            ...
            Cp = jrandom.uniform(keys[1],(batch_size,),minval=150,maxval=350)
